...
<script>
    ...
    export default {
        ...
        props: {
             identifier:  String/Object/Array ,
        ...
        asyncComputed: {
            async_field_list(){
                ...
                Object.keys( this.field_list ).forEach( ( field , i ) => {
                    console.log();
                    ...
                    if( ! self.is_undefined( this_field.rules.hide ) ) {
                        if( ! self.is_undefined( this_field.rules.show ) && self.is_object( this_field.rules.show ) ) {
                            Object.keys( this_field.rules.show ).forEach( ( value , x ) => {
        ...
        watch: {
            ...
            loaded_additional_form_data: function( val ){
        ...
        async mounted() {
            ...
            let load = await this.load_form_fields( this.loaded_identifier , this.loaded_area );
        },
        methods: {   
                        ...
                        if( ! self.is_undefined( this_field.rules.hide ) ) {
                            if( ! self.is_undefined( this_field.rules.show ) && self.is_array( this_field.rules.show ) ) {
                                this_field.rules.show.forEach( value , x => {
                                    if( ! self.is_undefined( field_list[x].rules.value ) && field_list[x].rules.value === value ){
                                        new_field_list[field] = this_field;
                                    }
                                });
                            }
            ...
            update_field( event ){
                console.log( 'UPDATE FIELD 1:', event );
                console.log( 'UPDATE FIELD 2:', this.snake_to_camel( event.validation ) );
                let _field_list = this.field_list;
            ...
            async load_form_fields( id , section ) {
                ...
                return this.request(
                    "post", 
                    "/axios/forms/load", 
                    this.merge_additional_load_form_data( { form_name: this.loaded_form_name , populate: this.populate , area: section , id: id } , this.loaded_additional_load_form_data ) ,
                     true , 
                     false 
                    ).then( response => {
            ...
            process_form: function(){
                ...
                this.request(
                    "post", 
                    "/axios/forms/process/" + this.loaded_form_data.area + "/" + this.loaded_form_data.name , 
                    this.merge_addition_data( this.form_data , this.loaded_additional_form_data ) , 
