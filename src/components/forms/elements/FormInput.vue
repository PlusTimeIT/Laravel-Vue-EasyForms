...
<template>
    <v-col :cols="cols" :offset="offset" class="pt-0 pb-0">
        <validation-provider v-slot="{ errors }" :name="field_data.validation" tag="div">
            <component 
            ...
            >
                ...
                <v-tooltip v-if="field_data.rules.help !== ''" slot="append" bottom>
                    <template #activator="{ on }">
                        <v-icon slot="activator" color="primary" dark v-on="on">mdi-help-box</v-icon>
                    </template>
...
import OperatingHours from "@components/shared/base/FormOperatingHours.vue";
import DatePicker from "@components/shared/base/FormDatePicker.vue";
import TimePicker from "@components/shared/base/FormTimePicker.vue";
import FacilitySelect from '@components/admin/base/FacilitySelect.vue';
import CustomerSelect from '@components/admin/base/CustomerSelect.vue';
import GeneratePasswordButton from '@components/admin/base/GeneratePasswordButton.vue';
import ZoneSelect from '@components/admin/base/ZoneSelect.vue';
import ServiceSelect from '@components/admin/base/ServiceSelect.vue';
...
export default {
    ...
    methods: {
        prepare_props( errors ) {
            ...
            if( ! this.is_undefined( this.field_data.rules.help ) && this.field_data.rules.help !== '' ) {
            ...
            if( ! this.is_undefined( this.field_data.rules.component ) && this.field_data.rules.component == 'date-picker' ) {
                ...
                if( ! this.is_undefined( this.field_data.rules.close_on_content_click ) ) 
                    result['close-on-content-click'] = this.field_data.rules.close_on_content_click;
